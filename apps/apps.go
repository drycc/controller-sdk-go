...
func List(c *drycc.Client, workspace string, results int) (api.Apps, int, error) {
	body, count, reqErr := c.LimitedRequest(fmt.Sprintf("/v2/apps/?workspace=%s", workspace), results)
